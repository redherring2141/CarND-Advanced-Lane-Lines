...
def process_frame(self, img_org, flag=False):    
	...
	img_pers, persM = corners_unwarp(img_undist, nx, ny, mtx, dist, src, dst)
	img_pers_gray = cv2.cvtColor(img_pers, cv2.COLOR_RGB2GRAY)

    img_ylane_th = lab_select(img_pers, thresh=(150, 200))
    img_wlane_th = luv_select(img_pers, thresh=(225, 255))
    img_grad_abx = abs_sobel_th(img_pers_gray, orient='x', ksize=ks, thresh=th)
    img_grad_mag = mag_sobel_th(img_pers_gray, ks, th)
    img_grad_dir = dir_sobel_th(img_pers_gray, ksize=7, thresh=(1.0, 1.2))
	
	roi_ylane_th = mask_roi(img_ylane_th)
	roi_wlane_th = mask_roi(img_wlane_th)
	roi_grad_abx = mask_roi(img_grad_abx)
	roi_grad_mag = mask_roi(img_grad_mag)

	roi_combined = np.zeros_like(img_grad_mag)
	roi_combined[(roi_ylane_th == 1) | (img_wlane == 1) |
				 (roi_grad_abx == 1) | (img_grad_mag == 1)] = 1
